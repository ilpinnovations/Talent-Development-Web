...
echo"<section id='main-content'>
	     ...
	     <h2><i class='fa fa-angle-right'></i> Top 3 Upcoming Sessions</h2>";
				 echo" <div class='row mtbox'>";
				  $result1 = mysql_query("SELECT * FROM session WHERE status='0' ORDER BY start_date ASC LIMIT 3", $talent_uk);
while ($row1 = mysql_fetch_assoc($result1)){
	$session_id1= $row1['session_id'];
	$name1=$row1['session_name'];
	$en_date1=$row1['end_date'];
	$en_date1 = date("D,j M",strtotime($en_date));
	$result9 = mysql_query("SELECT * FROM registered_sessions WHERE session_id='$session_id1'", $talent_uk);
if($result9)
{
...
$result10 = mysql_query("SELECT * FROM session_attend WHERE session_id='$session_id1' AND is_attend='1'", $talent_uk);
if($result10)
{
$attended_count= mysql_num_rows($result10);
$diff = intval($registered_count)-intval($attended_count);
}
$query2 = sprintf("SELECT count(*) FROM session_attend WHERE is_feedback='1' AND session_id='$session_id1'");
if($query2)
{
...
$total_feedbacks=$row2['count(*)'];
	...
	echo"<div class='col-lg-4 col-md-4 col-sm-4 mb'>
							<div class='weather-3 pn centered'>
								...
								<div class='info'>
									<div class='row'>
											<h3 class='centered'>".$registered_count." Registrations</h3>
										<div class='col-sm-6 col-xs-6 pull-left'>
											<p class='goleft'><i class='fa fa-tit'></i> &nbsp;</p>
										</div>
										<div class='col-sm-6 col-xs-6 pull-right'>
											<p class='goright'><i class='fa fa-fla'></i> &nbsp;</p>
										</div>
									</div>
								</div>
							</div>
